    ...
    public partial class FlightData : MyUserControl, IActivate, IDeactivate
    {
        ...
        public FlightData()
        {
            ...
            */
            ...
            // === ODIN-STYLE BOTTOM HUD OVERLAY ===
            // Must be called AFTER all event handlers and setup are done
            SetupOdinLayout();
        }
        ...
        // Floating instrument overlay widgets
        private Panel pnlCluster;
        ...
        {
            // If Odin layout is active, always keep Panel1 collapsed
            if (_odinLayoutActive)
            {
        ...
        {
            Panel card = new Panel { Dock = DockStyle.Fill, Height = 45, Padding = new Padding(2) };
            card.Paint += (s, e) => {
                Graphics g = e.Graphics;
                g.SmoothingMode = SmoothingMode.AntiAlias;

                // Rounded background for each card
                var rect = new Rectangle(4, 4, card.Width - 8, card.Height - 8);
                using (var gp = MainV2.RoundedRect(rect, 8))
                using (var bgBrush = new SolidBrush(Color.FromArgb(30, 35, 43))) // Lighter charcoal for card
                {
                ...
                {
                    g.DrawString(iconCode, iconFont, iconBrush, 12, 10);
                }

                g.DrawString(title, new Font("Segoe UI", 6.5F, FontStyle.Bold), new SolidBrush(Color.FromArgb(140, 150, 165)), 34, 6);
                g.DrawString(getValue(), new Font("Segoe UI", 9.5F, FontStyle.Bold), Brushes.White, 34, 16);
                g.DrawString(getSub(), new Font("Segoe UI", 6F), new SolidBrush(Color.FromArgb(100, 110, 125)), 34, 30);
            };
            return card;
        }
        ...
        private void SetupPages()
        {
            ...
            pnlMapToolbar.Location = new Point(pnlMapContainer.Width - 60, 70);
            pnlMapContainer.Resize += (s, e) => {
                pnlMapToolbar.Location = new Point(pnlMapContainer.Width - 60, 70);
            };


            lblBotAlt = new System.Windows.Forms.Label { Text = "0.0 m" };
            lblBotClimb = new System.Windows.Forms.Label { Text = "0.0 m/s" };
            lblBotGS = new System.Windows.Forms.Label { Text = "0.0 m/s" };
            lblBotDist = new System.Windows.Forms.Label { Text = "0.0 m" };
            lblBotHeading = new System.Windows.Forms.Label { Text = "0°" };
            lblBotRoll = new System.Windows.Forms.Label { Text = "0.0°" };
            lblBotPitch = new System.Windows.Forms.Label { Text = "0.0°" };
            lblBotYaw = new System.Windows.Forms.Label { Text = "0.0°" };
            lblBotFlightTime = new System.Windows.Forms.Label { Text = "00:00:00" };
            lblBotRcSignal = new System.Windows.Forms.Label { Text = "100%" };

            hudHorizon = new PictureBox { Visible = false };
            hudCompass = new PictureBox { Visible = false };
            hud = hudHorizon;

            // ══════════════════════════════════════════════════════════════════
            // UNIFIED INSTRUMENT CLUSTER (bottom-right corner)
            // ══════════════════════════════════════════════════════════════════
            const int W_SPEEDO  = 210;
            const int W_ATTI    = 370;
            const int W_COMPASS = 210;
            const int H_INSTR   = 210;
            const int H_TELEM   = 40;
            const int IGAP      = 6;
            const int IPAD      = 10;

            int clusterW = W_SPEEDO + IGAP + W_ATTI + IGAP + W_COMPASS + IPAD * 2;
            int clusterH = H_TELEM + IGAP + H_INSTR + IPAD * 2;
            int instrY = H_TELEM + IGAP + IPAD;

            // Outer container (hidden, we draw in gMapControl1.Paint)
            pnlCluster = new Panel {
                Width = clusterW, Height = clusterH,
                BackColor = Color.Transparent,
                Visible = false
            };

            // ── Telemetry strip (top row inside cluster) ──────────────────────
            var telemItems = new[] {
                ("ALT",   lblBotAlt),  ("SPEED", lblBotGS),   ("HDG",   lblBotHeading),
                ("CLIMB", lblBotClimb),("ROLL",  lblBotRoll),  ("PITCH", lblBotPitch),
                ("FT",    lblBotFlightTime), ("RC", lblBotRcSignal)
            };
            int colW = (clusterW - IPAD * 2) / telemItems.Length;
            for (int ti = 0; ti < telemItems.Length; ti++)
            {
                var (title, lbl) = telemItems[ti];
                int tx = IPAD + ti * colW;

                if (ti > 0)
                {
                    Panel div = new Panel {
                        Location = new Point(tx, 8), Width = 1, Height = H_TELEM - 16,
                        BackColor = Color.FromArgb(60, 70, 85)
                    };
                    pnlCluster.Controls.Add(div);
                }

                System.Windows.Forms.Label lblTitle = new System.Windows.Forms.Label {
                    Text = title, AutoSize = false,
                    Location = new Point(tx, 4), Size = new Size(colW, 14),
                    Font = new Font("Segoe UI", 6.5f, FontStyle.Regular),
                    ForeColor = Color.FromArgb(100, 130, 160),
                    TextAlign = ContentAlignment.MiddleCenter,
                    BackColor = Color.Transparent
                };
                pnlCluster.Controls.Add(lblTitle);

                lbl.AutoSize = false;
                lbl.Location = new Point(tx, 16);
                lbl.Size = new Size(colW, 22);
                lbl.Font = new Font("Segoe UI", 9.5f, FontStyle.Bold);
                lbl.ForeColor = Color.White;
                lbl.TextAlign = ContentAlignment.MiddleCenter;
                lbl.BackColor = Color.Transparent;
                pnlCluster.Controls.Add(lbl);
            }

            pnlMapContainer.Controls.Add(pnlCluster);

            Action repositionCluster = () =>
            {
                var parentW = gMapControl1 != null ? gMapControl1.Width : pnlMapContainer.Width;
                var parentH = gMapControl1 != null ? gMapControl1.Height : pnlMapContainer.Height;
                pnlCluster.Location = new Point(
                    parentW - clusterW - 10,
                    parentH - clusterH - 10);
            };
            repositionCluster();
            pnlMapContainer.Resize += (s, e) => repositionCluster();

            // Hook gMapControl1.Paint to draw HUD elements!
            if (gMapControl1 != null)
            {
                gMapControl1.Paint += (s, e) => {
                    var g = e.Graphics;
                    g.SmoothingMode = SmoothingMode.AntiAlias;
                    ...
                    int cY = gMapControl1.Height - clusterH - 10;
                    if (cX < 0) cX = 0;
                    if (cY < 0) cY = 0;
                    ...
                    // Telemetry pill bg
                    using (var gp = RoundedRect(new Rectangle(0, 0, clusterW, H_TELEM), 14))
                    using (var bg = new SolidBrush(Color.FromArgb(180, 20, 24, 30)))
                        g.FillPath(bg, gp);
                    ...
                    for(int i=0; i<botItems.Length; i++) {
                        int tx = IPAD + i * bottomColW;
                        if (i > 0) {
                            using(var pen = new Pen(Color.FromArgb(60, 70, 85)))
                    ...
                    DrawWidgetSpeedo(g, new Rectangle(IPAD, instrY, W_SPEEDO, H_INSTR));
                    DrawWidgetAtti(g, new Rectangle(IPAD + W_SPEEDO + IGAP, instrY, W_ATTI, H_INSTR));
                    DrawWidgetCompass(g, new Rectangle(IPAD + W_SPEEDO + IGAP + W_ATTI + IGAP, instrY, W_COMPASS, H_INSTR));
        ...
        private void UpdateTelemetry()
        {
            ...
            if (pnlCluster != null) { pnlCluster.BringToFront(); pnlCluster.Invalidate(); }
        ...
        private void DrawWidgetCompass(Graphics g, Rectangle bounds)
        {
            int W = bounds.Width, H = bounds.Height;
            int diam = Math.Min(W, H) - 4;
            int radius = diam / 2;
            var center = new PointF(bounds.X + W / 2f, bounds.Y + H / 2f);
            var rect   = new RectangleF(center.X - radius, center.Y - radius, diam, diam);

            // ── Bezel gradient ──
            using (var gp = new GraphicsPath()) {
                gp.AddEllipse(rect);
                using (var bgb = new System.Drawing.Drawing2D.PathGradientBrush(gp)) {
                    bgb.CenterColor    = Color.FromArgb(240, 24, 28, 34);
                    bgb.SurroundColors = new[] { Color.FromArgb(240, 8, 10, 14) };
                    g.FillPath(bgb, gp);
                }
            }

            // ── Tick marks (rotate dial opposite to heading) ──
            for (int deg = 0; deg < 360; deg += 5) {
                double a   = (deg - heading - 90.0) * Math.PI / 180.0;
                bool major = deg % 30 == 0;
                bool mid   = deg % 10 == 0;
                int  len   = major ? 10 : (mid ? 6 : 4);
                float r1 = radius - len - 4, r2 = radius - 4;
                var p1 = new PointF(center.X + (float)(Math.Cos(a) * r1),
                                    center.Y + (float)(Math.Sin(a) * r1));
                var p2 = new PointF(center.X + (float)(Math.Cos(a) * r2),
                                    center.Y + (float)(Math.Sin(a) * r2));
                using (var pen = new Pen(major ? Color.White : Color.FromArgb(160, 160, 160), major ? 1.5f : 1f))
            ...
            foreach (var (deg, lbl) in cardinals) {
                double a    = (deg - heading - 90.0) * Math.PI / 180.0;
                float  rTx  = radius - 26;
                using (var font = new Font("Segoe UI", 11f, FontStyle.Regular)) {
                    var    sz   = g.MeasureString(lbl, font);
                    var    pos  = new PointF(center.X + (float)(Math.Cos(a) * rTx) - sz.Width  / 2f,
                                             center.Y + (float)(Math.Sin(a) * rTx) - sz.Height / 2f);
                    g.DrawString(lbl, font, Brushes.White, pos);
                }
            }

            // ── Centre heading readout ──
            string hdg = $"{(int)heading:000}°";
            using (var f = new Font("Segoe UI", 28f, FontStyle.Regular)) {
                var sz = g.MeasureString(hdg, f);
                g.DrawString(hdg, f, Brushes.White, center.X - sz.Width / 2f, center.Y - sz.Height / 2f);
            }

            // ── Fixed red pointer at top ──
            using (var b = new SolidBrush(Color.FromArgb(239, 68, 68))) {
                var pts = new PointF[] {
                    new PointF(center.X, rect.Top + 6),
                    new PointF(center.X - 8, rect.Top - 6),
                    new PointF(center.X + 8, rect.Top - 6)
                };
                g.FillPolygon(b, pts);
            }
            ...
            // ── Outer ring ──
            using (var pen = new Pen(Color.FromArgb(80, 90, 110), 2f))
                g.DrawEllipse(pen, rect);
        }

        // ── SPEEDOMETER WIDGET ────────────────────────────────────────────────
        private void DrawWidgetSpeedo(Graphics g, Rectangle bounds)
        {
            var cs = MainV2.comPort?.MAV?.cs;
            float speed_kmh = (cs != null) ? (float)(cs.groundspeed * 3.6) : 0f;
            speed_kmh = Math.Max(0, Math.Min(100, speed_kmh));
            ...
            // ── Coloured arc (green→orange→red) ──
            const float START_ANGLE = 150f;   // bottom-left
            const float SWEEP_TOTAL = 240f;   // to bottom-right
            int arcR = radius - 3;
            var arcRect = new RectangleF(center.X - arcR, center.Y - arcR, arcR * 2, arcR * 2);

            // Green 0-50
            using (var pen = new Pen(Color.FromArgb(34, 197, 94), 4f))
                g.DrawArc(pen, arcRect, START_ANGLE, SWEEP_TOTAL * 0.50f);
            // Orange 50-75
            using (var pen = new Pen(Color.FromArgb(251, 146, 60), 4f))
                g.DrawArc(pen, arcRect, START_ANGLE + SWEEP_TOTAL * 0.50f, SWEEP_TOTAL * 0.25f);
            // Red 75-100
            using (var pen = new Pen(Color.FromArgb(239, 68, 68), 4f))
                g.DrawArc(pen, arcRect, START_ANGLE + SWEEP_TOTAL * 0.75f, SWEEP_TOTAL * 0.25f);

            // ── Speed tick marks + labels ──
            for (int s = 0; s <= 80; s += 10) {
                float ang  = (START_ANGLE + SWEEP_TOTAL * s / 80f) * (float)(Math.PI / 180.0);
                bool  maj  = s % 10 == 0;
                int   len  = 6;
                float r1 = radius - len - 6, r2 = radius - 6;
                var p1 = new PointF(center.X + (float)(Math.Cos(ang) * r1),
                                    center.Y + (float)(Math.Sin(ang) * r1));
                var p2 = new PointF(center.X + (float)(Math.Cos(ang) * r2),
                                    center.Y + (float)(Math.Sin(ang) * r2));
                using (var pen = new Pen(Color.White, maj ? 1.5f : 1f))
                    g.DrawLine(pen, p1, p2);
                if (maj) {
                    using (var f = new Font("Segoe UI", 8.5f, FontStyle.Bold)) {
                        ...
                        g.DrawString(txt, f, Brushes.White,
                            center.X + (float)(Math.Cos(ang) * rTx) - sz.Width  / 2f,
                            center.Y + (float)(Math.Sin(ang) * rTx) - sz.Height / 2f);
                    }
                }
            }

            // ── Blue Triangle Pointer ──
            float pointerAngle = (START_ANGLE + SWEEP_TOTAL * speed_kmh / 80f) * (float)(Math.PI / 180.0);
            float ptrR = radius - 14;
            var ptrTip = new PointF(center.X + (float)(Math.Cos(pointerAngle) * ptrR),
                                    center.Y + (float)(Math.Sin(pointerAngle) * ptrR));
            var ptrLeft = new PointF(center.X + (float)(Math.Cos(pointerAngle - 0.08) * (ptrR - 10)),
                                     center.Y + (float)(Math.Sin(pointerAngle - 0.08) * (ptrR - 10)));
            var ptrRight = new PointF(center.X + (float)(Math.Cos(pointerAngle + 0.08) * (ptrR - 10)),
                                      center.Y + (float)(Math.Sin(pointerAngle + 0.08) * (ptrR - 10)));
            using (var b = new SolidBrush(Color.FromArgb(58, 130, 246))) {
                g.FillPolygon(b, new PointF[] { ptrTip, ptrLeft, ptrRight });
            }

            // ── Centre readout ──
            using (var f = new Font("Segoe UI", 28f, FontStyle.Regular)) {
                var txt = ((int)speed_kmh).ToString();
                var sz  = g.MeasureString(txt, f);
                g.DrawString(txt, f, Brushes.White, center.X - sz.Width / 2f, center.Y - sz.Height / 2f - 6);
            }
            using (var f = new Font("Segoe UI", 9.5f, FontStyle.Bold)) {
                var sz = g.MeasureString("KM / h", f);
                g.DrawString("KM / h", f, Brushes.White, center.X - sz.Width / 2f, center.Y + 20);
            }
        }

        // ── ATTITUDE + ALTIMETER WIDGET (pill shape) ──────────────────────────
        private void DrawWidgetAtti(Graphics g, Rectangle bounds)
        {
            var cs = MainV2.comPort?.MAV?.cs;
            float alt_m = (cs != null) ? (float)cs.alt   : 0f;

            int W = bounds.Width, H = bounds.Height;
            int cornerR = H / 2;   // pill = full rounded ends

            // ── Pill background ──
            using (var gp = new GraphicsPath()) {
                gp.AddArc(bounds.X, bounds.Y, H, H, 90, 180);
                gp.AddArc(bounds.X + W - H, bounds.Y, H, H, -90, 180);
                gp.CloseFigure();
                using (var bgb = new SolidBrush(Color.FromArgb(240, 24, 28, 34)))
                    g.FillPath(bgb, gp);
                ...
                // ════════════════════════════════
                // LEFT HALF  ─ Altimeter tape
                // ════════════════════════════════
                int altTapeW = halfW;

                // Gradient sky background for alt section
                using (var sb = new SolidBrush(Color.FromArgb(240, 24, 28, 34)))
                    g.FillRectangle(sb, bounds.X, bounds.Y, altTapeW, H);

                // Tape: draw scale lines at round-10 m intervals
                float pixPerMeter = H / 200f;  // 200 m total visible range
                float midY = bounds.Y + H / 2f;
                for (int m = -120; m <= 120; m += 10) {
                    float y = midY - (m - alt_m) * pixPerMeter;
                    if (y < bounds.Y || y > bounds.Bottom) continue;
                    bool major = m % 50 == 0;
                    int  len   = major ? 18 : 10;
                    using (var pen = new Pen(Color.FromArgb(major ? 200 : 120, 200, 200, 200), major ? 1.5f : 1f))
                        g.DrawLine(pen, bounds.X + altTapeW - len - 2, y, bounds.X + altTapeW - 2, y);
                    if (major) {
                        string lbl = $"{m} m";
                        using (var f = new Font("Segoe UI", 6.5f)) {
                            var sz = g.MeasureString(lbl, f);
                            g.DrawString(lbl, f, Brushes.LightGray, bounds.X + altTapeW - len - sz.Width - 4, y - sz.Height / 2f);
                        }
                    }
                }

                // Centre red pointer line on altimeter
                using (var pen = new Pen(Color.FromArgb(220, 50, 50), 2f))
                    g.DrawLine(pen, bounds.X + altTapeW - 22, midY, bounds.X + altTapeW - 2, midY);

                // Altitude readout box (centre of alt section)
                string altStr = $"{(int)alt_m}";
                using (var f = new Font("Segoe UI", 24f, FontStyle.Regular)) {
                    var sz = g.MeasureString(altStr, f);
                    float bx = bounds.X + (altTapeW - sz.Width)  / 2f - 14;
                    float by = bounds.Y + (H        - sz.Height) / 2f;
                    g.DrawString(altStr, f, Brushes.White, bx, by);
                }
                using (var f = new Font("Segoe UI", 12f)) {
                    g.DrawString("m", f, Brushes.White, bounds.X + altTapeW / 2f + 14, midY - 10);
                }

                // Divider line
                using (var pen = new Pen(Color.FromArgb(60, 70, 85), 2f))
                    g.DrawLine(pen, bounds.X + halfW, bounds.Y + 20, bounds.X + halfW, bounds.Bottom - 20);

                // ════════════════════════════════
                // RIGHT HALF ─ Artificial Horizon
                // ════════════════════════════════
                int horiz_x = bounds.X + halfW;
                int horiz_w = W - halfW;
                int horiz_h = H;
                var horizRect = new Rectangle(horiz_x, bounds.Y, horiz_w, horiz_h);

                // clip to right half circle (pill end)
                using (var hClip = new GraphicsPath()) {
                    hClip.AddRectangle(horizRect);
                    g.SetClip(hClip, System.Drawing.Drawing2D.CombineMode.Intersect);
                }

                float cx = horiz_x + horiz_w / 2f;
                float cy = bounds.Y + horiz_h / 2f;

                // Save & apply rotation + pitch translation
                var state = g.Save();
                g.TranslateTransform(cx, cy);
                g.RotateTransform(-roll);
                g.TranslateTransform(0, pitch * 2.0f);

                // Sky
                using (var sb = new SolidBrush(Color.FromArgb(30, 120, 200)))
                    g.FillRectangle(sb, -horiz_w * 2, -horiz_h * 2, horiz_w * 4, horiz_h * 2);
                // Ground
                using (var sb = new SolidBrush(Color.FromArgb(80, 130, 30)))
                    g.FillRectangle(sb, -horiz_w * 2, 0, horiz_w * 4, horiz_h * 2);
                // Horizon line
                using (var pen = new Pen(Color.White, 2f))
                    g.DrawLine(pen, -horiz_w * 2, 0, horiz_w * 2, 0);
                ...
                using (var pen = new Pen(Color.White, 1.2f)) {
                    for (int p = -30; p <= 30; p += 10) {
                        if (p == 0) continue;
                        float py   = -(p * 2.0f);
                        float llen = p % 20 == 0 ? 20f : 12f;
                        g.DrawLine(pen, -llen, py, llen, py);
                        using (var f = new Font("Segoe UI", 5.5f)) {
                            string lbl = p.ToString();
                            g.DrawString(lbl, f, Brushes.White, llen + 2, py - 5);
                            g.DrawString(lbl, f, Brushes.White, -llen - 14, py - 5);
                        }
                    }
                }
                ...
                // Fixed aircraft symbol (centre of horizon section)
                using (var pen = new Pen(Color.FromArgb(220, 50, 50), 2.5f)) {
                    // left wing
                    g.DrawLine(pen, cx - 24, cy, cx - 8, cy);
                    // right wing
                    g.DrawLine(pen, cx + 8, cy, cx + 24, cy);
                    // centre body
                    g.DrawLine(pen, cx - 8, cy, cx, cy + 5);
                    g.DrawLine(pen, cx + 8, cy, cx, cy + 5);
                }
        ...
        // ── Legacy stubs (kept so any residual references compile) ─────────────
